...
extern kal_uint8 uem_L4c_module_id;
/***************************************************************************** 
****************************************************************************** 
* External Funcations 
******************************************************************************
...
/* under construction !*/
		...
		case MSG_ID_L4CUEM_GET_AUDIO_MUTE_STATUS_CNF:
			/* hold the local parameter struct and directly pass to L4c */
            hold_local_para((local_para_struct*) data_ptr);
            ilm_ptr->local_para_ptr = (local_para_struct*) data_ptr;
            SEND_ILM(MOD_UEM, uem_L4c_module_id, UEM_L4C_SAP, ilm_ptr);
			uem_L4c_module_id = 255;
			return;
...
/* under construction !*/
		...
		{
			media_aud_play_by_file_req_struct *dst_data_ptr;
			kal_wchar *file_name;
            dst_data_ptr = (media_aud_play_by_file_req_struct*)
                construct_local_para(sizeof(media_aud_play_by_file_req_struct), TD_CTRL);

            va_start(varpars, msg_name);
			dst_data_ptr->volume = (kal_uint64) va_arg(varpars, kal_uint64);
			file_name = (kal_wchar *)va_arg(varpars,kal_uint32);
            dst_data_ptr->play_style = (kal_uint8) va_arg(varpars, kal_uint32);
            dst_data_ptr->output_path = (kal_uint8) va_arg(varpars, kal_uint32);
            va_end(varpars);
			
            dst_data_ptr->start_offset= 0;
            dst_data_ptr->end_offset= 0;
			dst_data_ptr->cache_p = NULL;
            dst_data_ptr->identifier = 0;

			
			kal_wstrncpy(dst_data_ptr->file_name, file_name, MAX_MEDIA_FILE_NAME);
			kal_buffer_trace(TRACE_INFO, FUNC_UEM_SEND_MSG_TO_AUD, 60, (const kal_uint8*)file_name);

			kal_prompt_trace(MOD_UEM,"[wxw]play_by_file volume:%d play_style:%d file_name:%x output_path:%d",dst_data_ptr->volume,dst_data_ptr->play_style,file_name,dst_data_ptr->output_path);
            ilm_ptr->local_para_ptr = (local_para_struct*) dst_data_ptr;
            break;
		}
#endif
...
#ifdef __AUDIO_AT_CMD__
		{
            media_aud_stop_req_struct *dst_data_ptr;

            dst_data_ptr = (media_aud_stop_req_struct*) construct_local_para(sizeof(media_aud_stop_req_struct), TD_CTRL);
...
/* under construction !*/
		...
		{
			media_aud_set_speech_info_req_struct *dst_data_ptr;
			audio_speech_info_struct *new_speech_info;

			dst_data_ptr = (media_aud_set_speech_info_req_struct *)	
					construct_local_para(sizeof(media_aud_set_speech_info_req_struct), TD_CTRL);
			
			va_start(varpars, msg_name);					
            dst_data_ptr->src_id = (kal_uint8)va_arg(varpars, kal_uint32);
            new_speech_info = (audio_speech_info_struct *)va_arg(varpars, kal_uint32);
            va_end(varpars);
            //dst_data_ptr->update_nvram = KAL_TRUE;
            kal_mem_cpy((void *)&(dst_data_ptr->speech_info), new_speech_info, sizeof(audio_speech_info_struct));
            ilm_ptr->local_para_ptr = (local_para_struct*) dst_data_ptr;
