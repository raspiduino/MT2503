...
typedef enum
{
    LEVEL_TYPE = 0, // active (low) or de-active(high)
    PULSE_TYPE // repeated pulses (incoming call) or one  pulse (other RI event) 
}l4c_ri_signal_type;

typedef struct l4c_ri_signal_struct
{   
    kal_bool enable;
    l4c_ri_signal_type signaltype;
    kal_uint32 pulse_duration;// range is 5*10 ms - 10000*10 ms, unit is 10 ms
} l4c_ri_signal_struct;

typedef struct l4c_ri_cfg_struct
{
    l4c_ri_signal_struct INCOMING_CALL;  //URC:  +RING ,+CRING
    l4c_ri_signal_struct INCOMING_SMS;  //URC:  +CMT, +CMTI
    l4c_ri_signal_struct INCOMING_SMS_CB; //URC:  +CBM
    l4c_ri_signal_struct USSD;  //URC: +CUSD
    l4c_ri_signal_struct NETWORK_STATE; //URC:  +CIEV
    kal_bool ENABLE; //enable or disable the use of RI signal.  Default value: RI signal is used.
} l4c_ri_cfg_struct;

typedef enum
{
    DCD_DATA_CONNECTION_NULL = 0,  
	DCD_DATA_CONNECTION_PPP,   /* for dial up data call */
	DCD_DATA_CONNECTION_GPRS,  /* for at+cgact data call */
	DCD_DATA_CONNECTION_TCPIP  /* for tcpip bear data call */
}dcd_data_connection_flag;
#endif /* defined(__8_WIRE_UART_SUPPORT__) */
...
extern kal_uint8 plmn_convert_mnc_to_digit(plmn_id_struct source, kal_uint8 *dest);
...
#if defined(__8_WIRE_UART_SUPPORT__)
extern kal_bool l4c_set_ri_cfg_to_nvram(l4c_ri_cfg_struct *ri_cfg);
extern void l4c_get_ri_cfg_from_nvram(l4c_ri_cfg_struct *ri_cfg);
