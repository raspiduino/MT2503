...
void track_cust_gps_search_overtime_wifi_upload(void)
{
    ...
    {
        last_t2 = 0;
        track_stop_timer(TRACK_CUST_NO_FIX_SEND_WIFI_TIMER_ID);
        LOGD(L_GPS, L_V5, "GPS定位或关闭，无WIFI");
        return;
    }

    interval = track_cust_get_pack_time(); //GPS上报间隔的2倍
    if(interval == 0)
    {
        return;
    }
    ...
    if(last_t2 == 0)
    {
        ...
        tr_start_timer(TRACK_CUST_NO_FIX_SEND_WIFI_TIMER_ID, interval * 2000, track_cust_gps_search_overtime_wifi_upload);
        return;
    }
    ...
    if(t2 < (interval * 2))
    {
        LOGD(L_GPS, L_V5, "GPS不定位时间未超过");
        tr_start_timer(TRACK_CUST_NO_FIX_SEND_WIFI_TIMER_ID, (interval - t2) * 2000, track_cust_gps_search_overtime_wifi_upload);
        return;
    }
    ...
    if(t1 < interval)
    {
        ...
        tr_start_timer(TRACK_CUST_NO_FIX_SEND_WIFI_TIMER_ID, (interval - t1) * 1000, track_cust_gps_search_overtime_wifi_upload);
        return;
    }

    //gps不定位补wifi
    LOGD(L_GPS, L_V5, "gps不定位补wifi");
    track_cust_sendWIFI_paket();

    last_t1 = tick;
    tr_start_timer(TRACK_CUST_NO_FIX_SEND_WIFI_TIMER_ID, (interval + 5) * 1000, track_cust_gps_search_overtime_wifi_upload);
