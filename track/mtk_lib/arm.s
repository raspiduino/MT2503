;/*****************************************************************************
    ...
    IF :DEF: MT6223 :LOR: :DEF: MT6223P :LOR: :DEF: MT6225 :LOR: :DEF: M
...
_memcpy2_src_not_aligned
    SUBS    r2, r2, #4          ; if (length < 4) goto _memcpy_lastbytes
                                ; so we don't need to add 4 to r2 in ord
    BCC     _memcpy2_memcpy_lastbytes 
    LDR     r12, [r1, -r3]!
    CMP     r3, #2              ; if (src % 4 == 1) goto src_mod4_1
    BEQ     _memcpy2_src_mod4_2          ; if (src % 4 == 2) goto src_mo
    BHI     _memcpy2_src_mod4_3          ; if (src % 4 == 3) goto src_mo
_memcpy2_src_mod4_1
    MOV     r3, r12, LSR #8
    LDR     r12, [r1, #4]!
    SUBS    r2, r2, #4
...
_memcpy2_memcpy_lastbytes
    ...
    MOVS     r2, r2, LSL #31     ; if (length == 1) C = 0; N = 1; if (le
    LDRCSB  r3, [r1], #1    ; if (C == 1) ldrb  ; execute when length ==
    LDRCSB  r12, [r1], #1   ; if (C == 1) ldrb  ; execute when length ==
    LDRMIB  r2, [r1], #1    ; if (N == 1) ldrb  ; execute when length ==
...
__rt_memmove
    ...
    CMPCS   r2, r3          ; if (dest <= src || length <= (dest - src))
    BLS     __aeabi_memcpy 
    CMP     r2, #3
    ADD     r0, r0, r2
    ADD     r1, r1, r2
...
strcmp_nextword1
    LDR     r2, [r0], #4
    LDR     r3, [r1], #4
    SUB     r12, r2, lr         ; r12 = hasZeroByte(v) = (v - 0x01010101
    BIC     r12, r12, r2
    ANDS    r12, r12, lr, LSL #7 ; lr, LSL #7 => 0x80808080
    CMPEQ   r2, r3              ; if (hasZeroByte(r2) == 0) then compare
...
zero_or_inequal_detected
    ...
    TSTEQ   r12, #0xff0         ; if *s1 == 0, then r12(0xXXXXXX80) & 0x
                                ; because last 8 bits in r12 is 0x80, TS
                                ; because we want to clear the 'C' flag 
    BNE     strcmp_last0
    MOV     r0, r3, LSL #16
    SUBS    r0, r0, r2, LSL #16
    TSTEQ   r12, #0xff00        ; if *(s1 + 1) == 0, then r12 & 0xff00 =
    BNE     strcmp_last0
    MOV     r0, r3, LSL #8
    SUBS    r0, r0, r2, LSL #8
    TSTEQ   r12, #0xff0000      ; if *(s1 + 2) == 0, then r12 & 0xff0000
    SUBEQS  r0, r3, r2
strcmp_last0
    MOVNE   r0, r0, RRX
    BX      lr
...
strcmp_next_pair
    LDRB    r2, [r0], #1    ; r2 = *s1
    LDRB    r3, [r1], #1    ; r3 = *s2
    CMP     r2, #1          ; if r2(*s1) is 0, then r2(*s2) is not equal
    CMPCS   r2, r3          ; if r2(*s1) >= 1, then compare *s1 with *s2
    BNE     strcmp_last
    LDRB    r2, [r0], #1
    LDRB    r3, [r1], #1
    CMP     r2, #1          ; same as above
    CMPCS   r2, r3
    BEQ     strcmp_next_pair ; if (*s1 != 0 && *s1 == *s2) goto strcmp_n
strcmp_last
    SUB     r0, r2, r3
    BX      lr
...
__rt_memclr    
    MOV     r2,#0x0
_memset_8                      ;r0=>Target address, r1=>size(4-byte alig
    CMP     r1,#0x4                 
    BCC     _memset_8_last_bytes    
    ANDS    r12,r0,#0x3
    BEQ     _memset_w_8             ;If size >= 4 and address aligned 4-
    RSB     r12,r12,#0x4            ;Else Store 1 ~ 3 bytes to make addr
    CMP     r12,#0x2
    STRNEB  r2,[r0],#0x1
    SUB     r1,r1,r12
    STRGEH  r2,[r0],#0x2
    B       _memset_w_8             ;Finally goto memset_w
_memset_8_last_bytes                ;If size < 4, use STRB store 1 ~ 3 b
...
__rt_memclr_w 
    MOV     r2,#0x0
_memset_w_8                       ;r0=>Target address, r1=>size(4-byte a
    STMDB   r13!,{r14}
    MOV     r3,r2
    MOV     r12,r2
    MOV     r14,r2
    ...
    STMDB   r13!,{r4-r7}            ;If size >= 32, push 4 extra registe
    MOV     r4,r2
    MOV     r5,r2
    MOV     r6,r2
    MOV     r7,r2
_memset_w_8_store_32
    STMIA   r0!,{r2-r7,r12,r14}     ;And issue STM8 (longest possible da
    SUBS    r1,r1,#0x20
