...
close(FH_LIS);
	...
	#chengjun+fota
	if ( ( $adr_rw_load + 0x100 ) > 0x10FCFFFF )
	{
	    die "RW  节区超出长度了，需要修改！！";
	    exit 3;
	}
	...
	#chengjun+fota
	$adr_rw_exet = $adr_rw_exet + 0x10FCFF00 - $adr_rw_load ;
	$adr_rw_load = 0x10FCFF00;
	$string = sprintf("0x%x", $adr_rw_load );
	$string2 = sprintf("0x%x", $adr_rw_exet );
	#这个区域是卡位置用的，避免内存溢出	      
	print OFH ("RW_ADDR_$i $string\n");
	...
	#chengjun+fota
	#加载的NVRAM区域
	print OFH ("NVRAM_ADDR_0 0x10FD0038\n");
	print OFH ("{\n");
	print OFH ("    NVRAM_0 +0\n");
	print OFH ("    {\n");
