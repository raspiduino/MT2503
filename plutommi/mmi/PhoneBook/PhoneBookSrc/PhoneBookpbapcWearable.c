...
static MMI_BOOL mmi_phb_pbapc_is_connect(void);
...
void mmi_phb_pbapc_entry_list_tab(void)
{
    ...
    MMI_TRACE(MMI_PHB_PBAPC_TRACE_GROUP, TRC_MMI_PHB_PBAPC_ENTRY_LIST_TAB1, 0, phb_pbapc_cntx_p->total_pse_contact);
...
void mmi_phb_pbapc_entry_list(void)
{
    ...
    srv_phb_pbapc_get_btd_addr(&bt_addr,phb_pbapc_cntx_p->bt_index); 

    MMI_TRACE(MMI_PHB_PBAPC_TRACE_GROUP, TRC_MMI_PHB_PBAPC_ENTRY_LIST_V3_0,
        mmi_frm_is_in_backward_scenario(), srv_bt_cm_is_profile_connected_ex(SRV_BT_CM_PBAPC_CONNECTION, 0));
    phb_pbapc_cntx_p->list_state = 1; // active
    if (phb_pbapc_cntx_p->is_inbg[0] && !phb_pbapc_cntx_p->is_stop[phb_pbapc_cntx_p->bt_index])
    {
        mmi_frm_cb_reg_event(EVT_ID_PHB_PBAPC_SYNC_STATUS_UPDATE,mmi_phb_pbapc_update_sync_status_hdlr,NULL);
        mmi_phb_pbapc_show_loading(phb_pbapc_cntx_p->parent_id, SCR_ID_PHB_PROCESSING);
...
static void mmi_phb_pbapc_update_sync_status_hdlr(mmi_event_struct *evt)
{
    MMI_ID active_id;
    U8 percentage;
    srv_phb_sync_status_evt_struct *sync_data = (srv_phb_sync_status_evt_struct*)evt; 
    ...
    if (evt->evt_id == EVT_ID_PHB_PBAPC_SYNC_STATUS_UPDATE)
    {
        if (active_id == SCR_ID_PHB_PROCESSING)
        {
            ...
            /*if (percentage == 100)
            {
                mmi_phb_pbapc_show_loading_abort();
            }*/
...
static void mmi_phb_pbapc_stop_sync_op()
{
    ...
    if (phb_pbapc_cntx_p->is_inbg[index])
    {
        if (mmi_frm_group_is_present(GRP_ID_PHB_MAIN))
        {
...
static void mmi_phb_pbapc_entry_show_loading(void)
{
    ...
    index = phb_pbapc_cntx_p->bt_index;
    MMI_TRACE(MMI_PHB_PBAPC_TRACE_GROUP,TRC_MMI_PHB_PBAPC_SHOW_LOAD_V3, 0, index, phb_pbapc_cntx_p->is_inbg[index]);
    if (phb_pbapc_cntx_p->is_inbg[index])
    {
...
static S32 mmi_phb_pbapc_find_entry_internal(U8 *keyword)
{
    ...
    g_mmi_phb_pbapc_cntx.contact_count =
        (U16) srv_phb_pbapc_build_contact_list(phb_pbapc_cntx_p->bt_index, &qsearch_filter, g_mmi_phb_pbapc_cntx.contact_array, SRV_PHB_PBAPC_CONTACT_COUNT);
    result_count = g_mmi_phb_pbapc_cntx.contact_count;
...
static void mmi_phb_pbapc_show_pse_list(void)
{
    ...
    MMI_TRACE(MMI_PHB_PBAPC_TRACE_GROUP, TRC_MMI_PHB_PBAPC_BACKUP5, gui_buffer, phb_pbapc_cntx_p->contact_count);
    ...
    // reset gui buffer
    if(!is_connect || phb_pbapc_cntx_p->total_pse_contact == 0 || phb_pbapc_cntx_p->contact_count)// need change api
    {
        gui_buffer = NULL; 
    }
    MMI_TRACE(MMI_PHB_PBAPC_TRACE_GROUP, TRC_MMI_PHB_PBAPC_BACKUP9, is_connect);
   ...
   // }
...
#endif 
    {
        ...
        if (phb_pbapc_cntx_p->qsearch_input_buffer[0] == 0 || (phb_pbapc_cntx_p->qsearch_input_buffer[0] && phb_pbapc_cntx_p->contact_count))
        {
...
static S32 mmi_phb_pbapc_read_folder_cb(srv_phb_pbapc_load_contact_rsp *load_rsp, void *user_data)
{
    ...
    mmi_phb_pbapc_cntx_struct *phb_pbapc_cntx_p = NULL;
    phb_pbapc_cntx_p = mmi_phb_pbapc_get_cntx();
    cb_index = phb_pbapc_contx_cb_data->bt_index;
    index = phb_pbapc_cntx_p->bt_index;

    active_id = mmi_frm_scrn_get_active_id();
    MMI_TRACE(MMI_PHB_PBAPC_TRACE_GROUP, TRC_MMI_PHB_PBAPC_READ_CB, phb_pbapc_cntx_p->is_stop[index], phb_pbapc_cntx_p->is_inbg[index], load_rsp->ret);  

    phb_pbapc_cntx_p->is_inbg[index] = MMI_FALSE;
#ifdef __MTK_TARGET__
    wgui_status_icon_bar_hide_icon(STATUS_ICON_BT_SYNC_PB_BG);
#endif
...
static void mmi_phb_pbapc_sync_data()
{
    U8 index;
    mmi_phb_pbapc_cntx_struct *phb_pbapc_cntx_p = NULL;
    /*----------------------------------------------------------------*/
    ...
    srv_phb_pbapc_load_contact(phb_pbapc_cntx_p->bt_index, mmi_phb_pbapc_read_folder_cb, phb_pbapc_cntx_p);
}
...
static MMI_BOOL mmi_phb_pbapc_is_connect(void)
{
    ...
    MMI_TRACE(MMI_PHB_PBAPC_TRACE_GROUP, TRC_MMI_PHB_PBAPC_PSE_LIST_V3, phb_pbapc_cntx_p->bt_index, is_connect, addr, 0);
    return is_connect;
}
...
static void mmi_phb_pbapc_pse_list_connect(void)
{
    ...
    /*----------------------------------------------------------------*/
    phb_pbapc_cntx_p = mmi_phb_pbapc_get_cntx();
    srv_phb_pbapc_get_btd_addr(&addr, phb_pbapc_cntx_p->bt_index);
    ...
    MMI_TRACE(MMI_PHB_PBAPC_TRACE_GROUP, TRC_MMI_PHB_PBAPC_CONNECT_SINGLE, addr, phb_pbapc_cntx_p->bt_index,ret);

    if (ret)
    {
        
        // mmi_bt_dialer_conn_profile(NULL, gid, SRV_BT_CM_PBAPC_CONNECTION, mmi_phb_pbapc_con_profile_cb);
        srv_pbapc_connect(&addr, mmi_phb_pbapc_con_profile_cb);
...

{
    ...
    cui_bt_dialer_connecting_close(g_cui_id);
    if (!result)
    {
        mmi_frm_nmgr_popup(
...
static void mmi_phb_pbapc_covert_data(mmi_phb_pbapc_phb_info_struct *phb_info)
{
    ...
    for (i = 0; i < phb_pbapc_cntx_p->pse_phb.num_count; i++)
    {
...
mmi_ret mmi_phb_pbapc_notify_evt_hdlr(mmi_event_struct *evt)
{
    ...
    /*----------------------------------------------------------------*/
    phb_pbapc_cntx_p = mmi_phb_pbapc_get_cntx();
    MMI_TRACE(MMI_PHB_PBAPC_TRACE_GROUP, TRC_MMI_PHB_PBAPC_NOTIFY_V3, evt->evt_id, phb_pbapc_cntx_p->list_state);
...
static void cui_phb_pbapc_select_contact_entry(void)
{
    ...
    pbapc_tab_pages_info_array[tab_num].tab_icon = NULL;//the image icon not find.(U8*)GetImage(IMG_ID_PHB_TAB_LIST);
   ...
   // for (;tab_num < SRV_PBAPC_LINK_NUM; tab_num++)
    ...
    pbapc_tab_pages_info_array[2].screen_id = SCR_ID_PHB_PBAPC_BT1;
    pbapc_tab_pages_info_array[2].tab_entry_func = cui_phb_pbapc_select_contact_bt_entry;

#ifdef __MMI_PHB_UI_IN_TABS__
    pbapc_tab_pages_info_array[2].tab_icon = (U8*)GetImage(IMG_ID_PHB_TAB_BT_LIST);
    pbapc_tab_pages_info_array[2].tab_string = NULL;
#else
    pbapc_tab_pages_info_array[2].tab_icon = NULL;
    pbapc_tab_pages_info_array[2].tab_string = GetString(STR_ID_PHB_PBAPC_BT);
#endif*/
...
static void cui_phb_pbapc_pre_select_contact(void)
{
    ...
    MMI_TRACE(MMI_PHB_PBAPC_TRACE_GROUP,TRC_MMI_PHB_PBAPC_COVERT_DATA, phb_pbapc_cntx_p->bt_index, phb_pbapc_cntx_p->cui_pse_phb.num_count,phb_pbapc_cntx_p->cui_pse_phb.name[0], 1);
