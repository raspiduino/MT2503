...
// AAA add for DTMF length detect feature start
float timeCount1= 0;
float timeCount2= 0;
float timeLength =0;
int flag1 = -1;
int flag2 = -1;
// AAA add for DTMF length detect feature end
      ...
      if ( hdl->pauseCount >= CON_PAUSE_NUMBER ){
		 ...
		 /***************AAA************Edit Start************************************/
			if(timeCount1 > 0) {
			    if(flag1 ==1) {
			        if(flag2 == 0){
					    timeLength = timeCount1 - 1.2 + timeCount2;
				    }else {
				        timeLength = timeCount1- 0.6 + timeCount2-0.45;
				    }
			     }
		     }else {
			     timeLength = timeCount2 - 0.45;
		     }
			 timeCount1 = 0;
			 timeCount2 = 0;
...
/**
*
*AAA Add for geting KeyTone Length
*
*
*/
static int32 dtmfGetKeyLength(DTMF_Handle *hdl){

	return timeLength;
}
/**
*
*AAA Add for reseting time length.
*
*
*/
static void resetTime(DTMF_Handle *hdl) {
    timeLength = 0;
}
...
/***********AAA*********Edit Start*******************************/
	//printf("timeCount = %d\n",timeLength);
	timeLength = (timeLength * 106 * 20) / 160;
	if(timeLength <= 0) {
		timeLength = 0;
	}
/**************AAA********Edit End****************************************/
...
DTMF_Handle *DTMF_Open( int32 block_size, int32 block_no )
{
  ...
  /****************AAA Edit Start****************************************/
   hdl->GetKeyLength = dtmfGetKeyLength;
   hdl->resetTime = resetTime;
/******************AAA Edit End**************************************************/
   return hdl;
}
