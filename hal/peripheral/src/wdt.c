...
{
 static BOOT_UP_SOURCE  wdt_caused_source=INVLIAD_BOOT_UP;
	kal_uint8 wdt_retn_flag;
    ...
    if((__BtSS_mode_in_csci==AP_BTSS_HIBERNATION) && WDT_SW_RST)	
		{
		   g_Hibernation_to_ap_silent_reboot=KAL_TRUE;
		   AP_BtSS_set_mode(AP_BTSS_INVALID_MODE);

            if(g_TINY_CSD_Mem_Dump_Flag!=TINY_DUMP)
	     /*0.  WDT RST is caused by hibernation to ap resume  */
            	{
                     wdt_caused_source=HIBERNATION_RESUME;
                }
          else
	     /*1. WDT RST is caused by hibernation exception happened */
          	{
		   wdt_caused_source=HIBERNATION_EXCEPTION;
          	}
	...
	/*2. WDT RST is caused by hibernation hang  */
      else if((WDT_TIMEOUT_RST) &&  (0x01&&(wdt_retn_flag)))

        {
          WDT_CLR_RETN_FLAG(0x01);
          wdt_caused_source=TINY_SYSTEM_HANG;
	    }

	
	  else if ((__BtSS_mode_in_csci!=AP_BTSS_HIBERNATION) &&(!(0x01&&(wdt_retn_flag))) &&(g_TINY_CSD_Mem_Dump_Flag!=TINY_DUMP))
	  	{

          /*3. WDT RST is caused by  AP  system hang  */ 
			if((WDT_TIMEOUT_RST) && (!WDT_SW_RST))
            {
               wdt_caused_source =AP_TIMEOUT_RST;
			}
		/* 4. WDT RST is caused by  AP  systen software reset */
            else if ((!WDT_TIMEOUT_RST) && (WDT_SW_RST))
            {
              wdt_caused_source =AP_SW_RST;
			}
		/* 5.WDT RST should not be once  happend  when fisrt boot up */
			else if((!WDT_TIMEOUT_RST) && (!WDT_SW_RST))
			{
		        wdt_caused_source=FIRST_BOOT_UP;
			}

	  /*6. WDT  RST is caused by AP software and timeout reset*/
			else
				wdt_caused_source=AP_SW_AND_TIMEOUT_RST;
	  	}
	   else
	   	   wdt_caused_source=INVLIAD_BOOT_UP;

	     return wdt_caused_source;
