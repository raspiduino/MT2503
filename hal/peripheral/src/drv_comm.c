...
extern void UART_EINT_wake_up_init();
...
extern void SPPA_UART_Handler_CB_init(void);
...
#ifdef __8_WIRE_UART_SUPPORT__
   DCL_HANDLE extuart_handle;
   extern Seriport_HANDLER_T ExtUart_Drv_Handler;
   kal_char extuart_buff[100];
#endif /* __8_WIRE_UART_SUPPORT__ */
...
#if defined(__TINY_SYS__)
   boot_reason = AP_check_boot_up_source();
   csci_init(boot_reason==FIRST_BOOT_UP);   
#endif
...
#ifdef __8_WIRE_UART_SUPPORT__
   sprintf(extuart_buff,"exuart start");
   kal_bootup_print(extuart_buff);
   extuart_handle =  DclExtUart_Open(uart_port1, 0);
   DclExtUart_RegisterCallback(extuart_handle, &ExtUart_Drv_Handler);
   DclExtUart_Control(extuart_handle,SWUART_CMD_INIT,NULL);
#endif /* __8_WIRE_UART_SUPPORT__ */
