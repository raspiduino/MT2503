...
/******************************************************************************
 *  Function    -  track_GPIO_SetClkOut
 *  Purpose     -  设置GPIO输出
 *  当gpio_clk_mode mode为mode_max_num设置GPIO21的输出电流，这种方法不太好，但不用在ELL中增加函数
 *  Description -  a
 *  modification history
 * ----------------------------------------
 * v1.0  , 05-12-2013,  wangqi  written 会引起OTA编译不过
 * ----------------------------------------
 ******************************************************************************/
void track_GPIO_SetClkOut(char clk_num, gpio_clk_mode mode)
{
    DCL_HANDLE handle;
    ...
    //kal_prompt_trace(2,"初始化CLK OK,mode_max_num:%d,clk_num:%d,mode:%d",mode_max_num, clk_num, mode);
}
#endif
